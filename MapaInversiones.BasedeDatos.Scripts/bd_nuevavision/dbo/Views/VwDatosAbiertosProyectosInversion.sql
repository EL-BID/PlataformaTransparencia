...
SELECT proy.CodigoBPIN AS CodigoSNIP, Est.NombreEstado AS Estado, proy.IdSector, sec.NombreSector, proy.FechaInicioProyecto, proy.FechaFinProyecto, EntidadEjecutora.NombreEntidad AS EjecutorPrimario, proy.NombreProyecto, proy.VlrTotalProyectoTodasLasFuentes AS CostoEstimadoProyecto, proy.PorcentajeAvanceFinanciero AS AvanceFinanciero, proy.PorcentajeAvanceFisico AS AvanceFisico, ISNULL(EntidadEjecutora.IdEntidad, 12001) AS IdEntidadEjecutora, 
         ISNULL(EntidadEjecutora.NombreEntidad, 'PRESIDENCIA DE LA REPÚBLICA') AS EntidadEjecutora, dbo.duration(proy.FechaInicioProyecto, proy.FechaFinProyecto) AS DuracionProyecto, (CASE WHEN proy.NombreOCAD = 'NA' THEN 'NACIONAL' WHEN proy.NombreOCAD = 'DE' THEN 'DEPARTAMENTAL' WHEN proy.NombreOCAD = 'MU' THEN 'MUNICIPAL' ELSE 'NACIONAL' END) AS TipoProyecto, ***REMOVED*** fn CONCAT('[', 
         ***REMOVED*** fn CONCAT(FORMAT(informeAvanceFisico.FechaInicioReporte, 'dd-MM-yyyy', 'en-US'), ***REMOVED*** fn CONCAT(' - ', ***REMOVED*** fn CONCAT(FORMAT(informeAvanceFisico.FechaFinReporte, 'dd-MM-yyyy', 'en-US'), ***REMOVED*** fn CONCAT('] --- ', ISNULL(informeAvanceFisico.Descripcion, 'No hay informe relacionado')) ***REMOVED***) ***REMOVED***) ***REMOVED***) ***REMOVED***) ***REMOVED*** AS InformeAvance, pxe.IdDepartamento AS IdDepartamentoLocalizacion, dbo.Departamento.NombreDepartamento AS DepartamentoLocalizacion, 
         pxe.IdMunicipio AS IdMunicipioLocalizacion, dbo.Municipio.NombreMunicipio AS NombreMunicipioLocalizacion, dbo.FuenteDeLosRecursos.FechaActualizacionFuente AS FechaCargueArchivo, ***REMOVED*** fn CONCAT(dbo.FuenteDeLosRecursos.Descripcion, ' - Ministerio de Hacienda') ***REMOVED*** AS FuenteDatos
